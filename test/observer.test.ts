...
import { Files, Processes, probePid } from "../src/io.js";
import { observe } from "../src/observer.js";
import { configEffect, fixture, platform, route } from "./helpers.js";
...
it.effect(
  "EPERM means alive; ESRCH means dead; unexpected probe failures remain unknown",
  () =>
    Effect.gen(function* () {
...
it.effect(
  "filters dead PIDs and defaults to excluding unverified aliases",
  () =>
    Effect.gen(function* () {
...
it.effect(
  "does not probe foreign PID namespaces; aliases require explicit opt-in",
  () =>
    Effect.gen(function* () {
      const config = yield* configEffect;
      const result = yield* observe(
        ...
        Effect.provideService(Files, {
          read: () =>
            Effect.succeed(
              JSON.stringify([route(), route("alias.dev.example.com", 0)]),
            ),
        }),
...
it.effect(
  "retries partial/unstable writes and only accepts a matching pair",
  () =>
    Effect.gen(function* () {
      const config = yield* configEffect;
