...
import type { Inventory } from "../src/dns.js";
import { inNamespace, parseConfig, privateAddress } from "../src/model.js";
import { configEffect, sample } from "./helpers.js";
  ...
  it.effect(`rejects unsafe configuration: ${JSON.stringify(patch)}`, () =>
    Effect.gen(function* () {
      const result = yield* Effect.result(
        parseConfig(JSON.stringify({ ...sample, ...patch })),
      );
...
it.effect(
  "groups parallel worktrees and dotted names without flattening or overbroad TLS",
  () =>
    Effect.gen(function* () {
      const config = yield* configEffect;
      const hosts = [
      ...
      expect(desiredDns(config, [...hosts].reverse())).toEqual(
        desiredDns(config, hosts),
      );
    }),
);

it.effect(
  "unconfigured DNS is unknown, never an empty authoritative inventory",
  () =>
    Effect.gen(function* () {
      const config = yield* configEffect;
      const desired = desiredDns(config, ["app.dev.example.com"]);
      expect(
        diffDns(config, desired).every((c) => c.action === "ensure-unverified"),
      ).toBe(true);
    }),
);
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(
        diffDns(config, desired, empty).every((c) => c.action === "create"),
      ).toBe(true);
      const owned: Inventory = { ...empty, records: desired };
      expect(
        diffDns(config, desired, owned).every((c) => c.action === "unchanged"),
      ).toBe(true);
      ...
      expect(
        diffDns(config, desired, unmanaged).every(
          (c) => c.action === "satisfied-unmanaged",
        ),
      ).toBe(true);
      ...
      expect(
        diffDns(config, desired, conflicts).every(
          (c) => c.action === "blocked",
        ),
      ).toBe(true);
      ...
      expect(
        diffDns(config, desired, update).every(
          (c) => c.action === "update-owned",
        ),
      ).toBe(true);
...
it.effect(
  "blocks CNAME, opposite-family address conflicts and delegated subzones",
  () =>
    Effect.gen(function* () {
      const config = yield* configEffect;
      const desired = desiredDns(config, ["app.dev.example.com"]);
      for (const type of ["CNAME", "AAAA", "NS"]) {
        ...
        expect(
          result.find((c) => c.record.name === "app.dev.example.com")?.action,
        ).toBe("blocked");
        expect(JSON.stringify(result)).not.toContain("secret-canary");
      }
    }),
...
it.effect(
  "accepts real DNS owner grammar without admitting those names as HTTP routes",
  () =>
    Effect.gen(function* () {
      const config = yield* configEffect;
      const records = [
      ...
      for (const record of records)
        expect(inNamespace(record.name, config.namespace)).toBe(false);
      expect(
        diffDns(
          config,
          desiredDns(config, ["app.dev.example.com"]),
          inventory,
        ).every((c) => c.action === "create"),
      ).toBe(true);
    }),
);
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(
        changes.filter((c) => c.action === "blocked").map((c) => c.record.name),
      ).toEqual(["*.app.dev.example.com", "feat.app.dev.example.com"]);
      expect(
        changes.find((c) => c.record.name === "app.dev.example.com")?.action,
      ).toBe("create");
    }),
);

it.effect(
  "uses exact-only certificates where a wildcard would exceed 253 characters",
  () =>
    Effect.gen(function* () {
      const namespace = `${"a".repeat(63)}.${"b".repeat(63)}.${"c".repeat(63)}.${"d".repeat(58)}`;
      const config = yield* parseConfig(
        JSON.stringify({ ...sample, namespace }),
      );
      for (const label of ["x", "xx"]) {
        ...
        expect(certificateGroups([host], namespace)[0]?.subjects).toEqual([
          host,
        ]);
...
it.effect(
  "rejects malformed or wrong-zone inventory rather than inventing changes",
  () =>
    Effect.gen(function* () {
      const config = yield* configEffect;
      const records = [
        { name: "app.dev.example.com", type: "A", values: ["10.1.2.3"] },
      ];
      for (const value of [
        "{secret-canary",
