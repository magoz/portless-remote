...
import { emptyIntent } from "../src/model.js";
import { Files, MAX_FILE_BYTES } from "../src/io.js";
import { IntentStore, intentStoreLayer } from "../src/state.js";
import { fixture, platform } from "./helpers.js";
...
it.live(
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(
        (yield* fs.stat(path.join(config.dataDir, "intent.json"))).mode & 0o777,
      ).toBe(0o600);
...
it.live(
  "fails closed on corrupt state and mismatched identity without overwriting evidence",
  () =>
    Effect.gen(function* () {
...
it.live(
  "refuses overlapping symlink paths before creating anything in Portless state",
  () =>
    Effect.gen(function* () {
...
it.live("rejects symlinked intent and shared-writable data directories", () =>
  Effect.gen(function* () {
    ...
    expect(
      (yield* Effect.result(
        load.pipe(Effect.provide(intentStoreLayer(config))),
      ))._tag,
    ).toBe("Failure");
...
it.live("bounds file reads and treats missing files separately", () =>
  Effect.gen(function* () {
    ...
    expect(missing._tag === "Failure" && missing.failure.code).toBe(
      "file-missing",
    );
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        const result = yield* Effect.result(
          store.save({ ...previous, retainedHosts }),
        );
        expect(result._tag === "Failure" && result.failure.code).toBe(
          "intent-size-limit",
        );
...
it.live(
  "releases the cross-process lock when the owning scope is interrupted",
  () =>
    Effect.gen(function* () {
      const { fs, path, config } = yield* fixture;
