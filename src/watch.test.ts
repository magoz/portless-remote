...
import { configEffect, fixture, memoryServices, platform, route } from '#app/testing/fixtures'

it.effect('renews scoped watchers, polls, deduplicates output, and cancels all resources', () =>
  Effect.gen(function* () {
    ...
    const fiber = yield* Effect.forkScoped(
      watch(config, r.run.pipe(Effect.tap(() => Ref.update(passes, n => n + 1))), () =>
...
it.live(
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* fs.writeFileString(registry, JSON.stringify([route('unseen.dev.example.com')]))
