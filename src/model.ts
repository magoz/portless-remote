...
const ConfigSchema = Schema.Struct({
  ...
  pollIntervalMs: Schema.Int.check(
    Schema.isBetween({ minimum: 250, maximum: 60_000 }),
  ).pipe(Schema.withDecodingDefaultKey(Effect.succeed(5_000))),
  maxHosts: Schema.Int.check(
    Schema.isBetween({ minimum: 1, maximum: 10_000 }),
  ).pipe(Schema.withDecodingDefaultKey(Effect.succeed(1_000))),
