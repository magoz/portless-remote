...
export class Files extends Context.Service<
  Files,
  {
    readonly read: (path: string, noFollow?: boolean) => Effect.Effect<string, BoundaryError>
  }
  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const read = Effect.fn('Files.read')(
        function* (path: string, noFollow = false) {
          if (noFollow) {
            ...
            if (link._tag === 'Success') {
              return yield* Effect.fail(new BoundaryError({ code: 'file-symlink-rejected' }))
            }
          ...
          if (stat.type !== 'File' || stat.size > BigInt(MAX_FILE_BYTES)) {
            return yield* Effect.fail(new BoundaryError({ code: 'file-unsupported-or-oversized' }))
          }
        ...
        Effect.scoped,
        Effect.catchTag('PlatformError', error => Effect.fail(fileError(error)))
      )
...
export const probePid = (
  ...
  Effect.try({
    try: () => {
      ...
      return code === 'ESRCH' ? 'dead' : code === 'EPERM' ? 'permission-denied' : 'unknown'
    }
