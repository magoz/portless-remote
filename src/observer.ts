...
import { BoundaryError, inNamespace, RouteSchema, sortedUnique } from '#app/model'
...
export const observe = Effect.fn('Observer.observe')(function* (config: Config, settleMs = 75) {
  ...
  const readStable = Effect.gen(function* () {
    ...
    if (first !== second)
      return yield* Effect.fail(new BoundaryError({ code: 'registry-unstable' }))
    // Ignore optional Portless transport metadata. It is never logged or persisted.
    return yield* Schema.decodeUnknownEffect(Schema.fromJsonString(Schema.Array(RouteSchema)))(
      second
    ).pipe(Effect.mapError(() => new BoundaryError({ code: 'registry-invalid' })))
  })
  ...
  for (const [index, route] of routes.entries()) {
    ...
    if (seen.has(route.hostname))
      return yield* Effect.fail(new BoundaryError({ code: 'registry-duplicate-host' }))
    seen.add(route.hostname)
    if (route.pid === 0) {
      notices.push({
        index,
        code: config.includePersistentAliases ? 'alias-liveness-unverified' : 'alias-excluded'
      })
