      ...
      (inventory) => {
        ...
        return (
          ...
          inventory.records.every(
            (r) => validDnsOwner(r.name) && r.values.length > 0,
          )
        );
      },
      ...
      return {
        read:
          path === undefined
            ...
            : files
                .read(path)
                .pipe(Effect.flatMap((text) => parseInventory(config, text))),
      };
    }),
  );
...
export const certificateGroups = (
  ...
  const bases = sortedUnique(
    hosts.map((host) => {
      ...
      return relativeLabels.length === 1
        ? host
...
export const diffDns = (
  ...
  desired.map((record) => {
    const plain = record.name.startsWith("*.")
      ? record.name.slice(2)
      : record.name;
    ...
    if (
      inventory.records.some(
        (r) => r.type === "DNAME" && record.name.endsWith(`.${r.name}`),
      )
    ) {
      return {
        record,
    ...
    if (
      atName.some(
        (r) =>
          r.type === "CNAME" ||
          ((r.type === "A" || r.type === "AAAA") && r.type !== record.type),
      )
    ) {
      return {
        record,
    ...
    const same =
      JSON.stringify(sortedUnique(existing.values)) ===
      JSON.stringify(record.values);
    const owned = existing.ownerId === config.ownerId;
    if (same)
      return { record, action: owned ? "unchanged" : "satisfied-unmanaged" };
