...
import { fixture, platform, route } from '#app/testing/fixtures'
...
const execute = Effect.fn(function* (
  ...
  const processHandle = yield* spawner.spawn(
    ChildProcess.make(
      process.execPath,
      [
        ...(mode === 'source' ? ['--conditions=development', 'src/cli.ts'] : ['dist/cli.js']),
        ...args
      ],
      {
...
for (const mode of ['compiled', 'source'] as const)
  it.live(
    `${mode} CLI dry-run and status observe isolated real state, persist intent and report not-ready`,
    () =>
      Effect.gen(function* () {
        ...
        const first = yield* execute(['dry-run', '--config', configFile], mode)
        ...
        const second = yield* execute(['status', '--config', configFile], mode)
...
it.live(
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* fs.writeFileString(file, JSON.stringify({ ...config, token: 'secret-canary' }))
      ...
      expect((yield* execute(['dry-run', '--config', file, '--apply'])).code).not.toBe(0)
    ...
    () =>
      Effect.gen(function* () {
        ...
        const handle = yield* spawner.spawn(
          ChildProcess.make(process.execPath, ['dist/cli.js', 'watch', '--config', configFile], {
            env: { NO_COLOR: '1' },
            extendEnv: false
          })
        )
        ...
        expect(JSON.parse(first[0]!).observation.hosts).toEqual(['app.dev.example.com'])
        ...
        expect(exit._tag).toBe(killSignal === 'SIGKILL' ? 'Failure' : 'Success')
        ...
        if (killSignal === 'SIGKILL') {
          expect((yield* execute(['status', '--config', configFile])).code).not.toBe(0)
          yield* fs.remove(lock, { recursive: true }) // deliberately recover our isolated fixture
          expect((yield* execute(['status', '--config', configFile])).code).toBe(0)
        }
