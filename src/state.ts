...
import { Files, MAX_FILE_BYTES } from "./io.js";
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const data = yield* Effect.gen(function* () {
        const portless = yield* canonicalPath(
          path.resolve(config.portlessStateDir),
        );
        const data = yield* canonicalPath(path.resolve(config.dataDir));
        const caFile = yield* canonicalPath(
          path.resolve(config.portlessCaFile),
        );
        const inventoryFile =
        ...
        if (
          ...
          return yield* Effect.fail(
            new BoundaryError({ code: "data-directory-overlaps-input" }),
          );
        }
        ...
        if (
          (stat.mode & 0o077) !== 0 ||
          (process.getuid &&
            Option.getOrUndefined(stat.uid) !== process.getuid())
        ) {
          return yield* Effect.fail(
            new BoundaryError({ code: "data-directory-not-private" }),
          );
        }
        return data;
      }).pipe(
        Effect.catchTag("PlatformError", () =>
          Effect.fail(
            new BoundaryError({ code: "data-directory-unavailable" }),
          ),
        ),
      );
      ...
      yield* Effect.acquireRelease(
        fs
          ...
          .pipe(
            Effect.mapError(
              () => new BoundaryError({ code: "intent-lock-unavailable" }),
            ),
          ),
        () =>
          fs.remove(lockPath, { recursive: true }).pipe(
            Effect.mapError(
              () => new BoundaryError({ code: "intent-lock-release-failed" }),
            ),
            Effect.orDie,
          ),
      );
      ...
      const load = files.read(statePath, true).pipe(
        Effect.flatMap((text) =>
          decodeJson(IntentSchema, text, "intent-invalid"),
        ),
        Effect.catchIf(
      ...
      const save = Effect.fn("IntentStore.save")(
        function* (intent: Intent) {
          ...
          if (encoded.length > MAX_FILE_BYTES)
            return yield* Effect.fail(
              new BoundaryError({ code: "intent-size-limit" }),
            );
